...
import { SITE, PROJECT_URLS, shouldEmbedInPortfolioIframe, isBlockedGithubComUrl } from '../config/site'
...
function OpenProject() {
  ...
  const allIEPRojects = ['BisAI', 'SEPMS', 'Acadex', 'AiAgent', '3dObject', 'Raasta', 'Araaaz']
  ...
  function handleFetchLinkDes(projectName) {
    switch(projectName) {
    case 'BisAI':
      return PROJECT_URLS.BisAI;

    case 'SEPMS':
      return PROJECT_URLS.SEPMS;

    case 'Acadex':
      return PROJECT_URLS.Acadex;

    case 'AiAgent': 
        return SITE.githubProfile;

    case '3dObject': 
        return PROJECT_URLS.object3D; 
        

    case 'Raasta':
        return PROJECT_URLS.Raasta;

    case 'Araaaz':
        return PROJECT_URLS.Araaaz;
    ...
    <>
      ...
      >
        ...
        {openProjectExpand.show && (
          shouldEmbedInPortfolioIframe(projectUrl) ? (
          <iframe
          key={iframeKey}
          src={projectUrl}
          width="100%"
        ...
        />
            ...
            <div className="openproject_embed_fallback">
              <p className="openproject_embed_fallback_title">
                {isBlockedGithubComUrl(projectUrl) ? 'GitHub cannot open here' : 'Cannot show in this window'}
              </p>
              <p className="openproject_embed_fallback_text">
                {isBlockedGithubComUrl(projectUrl)
                  ? 'github.com blocks embedding on every account (not just yours). Use the button to open your profile or repo in a full tab.'
                  : 'This page blocked embedding. Open it in your browser instead.'}
              </p>
              <button
                type="button"
                className="openproject_embed_fallback_btn"
                onClick={(e) => {
                  e.stopPropagation();
